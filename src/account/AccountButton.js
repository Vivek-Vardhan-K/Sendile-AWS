...
function AccountButton(props) {
  ...
  const devElem = devices.map((elem) => {
    ...
    return (
      <button
        ...
        key={elem.deviceId}
        onClick={() => {
          defaultDeviceHandler(elem.deviceId);
        }}
      >
        {elem.deviceType == 1 && <img src={ipadLogo} className="but-img"></img>}
        {elem.deviceType == 2 && (
          <img src={androidLogo} className="but-img"></img>
        )}
        {elem.deviceType == 3 && (
          <img src={appleLogo} className="but-img"></img>
        )}
        {elem.deviceType == 4 && (
          <img src={ereaderLogo} className="but-img"></img>
        )}
  ...
  return (
    <div>
      <img
        src={googleDetail.picture}
        className={props.page == "search" ? "prof-img" : "prof-img-res"}
        onClick={handleShow}
      />
      <Modal show={show} onHide={handleClose} className="ovrd-modal">
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleManageDevices}>
            Manage devices
          </Button>
        </Modal.Footer>
      </Modal>
